...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($productsIdSorted as $singleProductId){
            $product = Product::with('productItems','productImages','productItems.productItemFeatures')->find($singleProductId);
        ...
        foreach ($productsIdSorted as $singleProductId){
            $product = Product::with('productImages','productItems.productItemFeatures','productItems')->find($singleProductId);
            $product['selling_count'] = $productsCountCache[$singleProductId];
            array_push($trendingWeeklyProducts,$product);
        }

        $cart_count = Cart::where('user_id', auth()->user()->id)->where('active','=','1')->count();
