...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($name){
            $products = Product::with('productImages','productItems.productItemFeatures')->where('name','LIKE','%'.$name.'%')->paginate(7);
        }else{
            $products = Product::with('productImages','productItems.productItemFeatures')->paginate(12);
